...
public class M81R06XLSView extends AbstractPOIExcelView {
	...
	protected void buildExcelDocument(Map<String, Object> model,
		...
		List<Object[]> targetValues = (List<Object[]>) model.get("targetValues");
		// now turn targetValues into Map
		HashMap<Long, Object[]> targetValuesMap = new HashMap<Long, Object[]>();
		for(Object[] value : targetValues) {
			Long targetId = (Long) value[0];
			targetValuesMap.put(targetId, value);
		}
		...
		while(!objStack.empty()) {
			...
			if(o.getShowInTree()) {
				...
				if(o.getType().getId() == 106L) {
					...
					// just show empty cell
					for(int k=1; k<6; k++) {
						Cell blankCell = row.createCell(k);
						blankCell.setCellStyle(styles.get("cellleft"));
					}
					
					Stack<Activity> actStack = new Stack<Activity>();
					if(o.getFilterActivities() != null) {
						Collections.sort(o.getFilterActivities(), Activity.Comparators.CODE_DESC);
						actStack.addAll(o.getFilterActivities());
						
						while(!actStack.empty()) {
							
							Activity act = actStack.pop();
							
							if(act.getFilterTargets() != null && act.getFilterTargets().size() > 0) {
								for(ActivityTarget t : act.getFilterTargets()) {
									Row rowAct = sheet.createRow(i++);
									Cell cellAct = rowAct.createCell(0);
									cellAct.setCellValue("         -" + act.getName() + " (" + t.getUnit().getName() + ") ");
									
									Cell allTargetValue = rowAct.createCell(1);
									allTargetValue.setCellValue(t.getTargetValue());
									allTargetValue.setCellStyle(styles.get("cellnumber"));
									Object[] results = targetValuesMap.get(t.getId());
									
									Cell planTargetValue = rowAct.createCell(2);
									planTargetValue.setCellValue((Double) results[1]);
									planTargetValue.setCellStyle(styles.get("cellnumber"));
									
									Cell resultTargetValue = rowAct.createCell(3);
									resultTargetValue.setCellValue((Double) results[2]);
									resultTargetValue.setCellStyle(styles.get("cellnumber"));
									
									Cell percentMonthTargetValue = rowAct.createCell(4);
									percentMonthTargetValue.setCellValue(((Double) results[1] / (t.getTargetValue())) * 100);
									percentMonthTargetValue.setCellStyle(styles.get("cellpercent"));
	
									Cell percentAllTargetValue = rowAct.createCell(5);
									percentAllTargetValue.setCellValue(((Double) results[2] / (t.getTargetValue())) * 100);
									percentAllTargetValue.setCellStyle(styles.get("cellpercent"));
								...
								Row rowAct = sheet.createRow(i++);
								Cell cellAct = rowAct.createCell(0);
								cellAct.setCellValue("         -" +act.getName());
								
								for(int k=1; k<6; k++) {
									Cell blankCell = rowAct.createCell(k);
									blankCell.setCellStyle(styles.get("cellleft"));
								}
								
							}
							Collections.sort(act.getChildren(), Activity.Comparators.CODE_DESC);
							actStack.addAll(act.getChildren());
						}
					} 
